...
description: Initialize and operate a personal AI engineering harness for a repository. Use when Claude needs to identify the repository profile, bootstrap a minimum `.ai` project framework, enforce the hard rules "never say complete before verification" and "only one active feature at a time", and move the user into the standard feature delivery workflow.
---

# Personal Harness

Use this skill to turn an ordinary repository into a controlled AI engineering workspace inside Claude Code.
